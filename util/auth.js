...
const checkAccess = (req, res, next) => {
   ...
   * ];
   */
  const MANAGERS = ['admin', 'defcoord'];
  const SCOUTS = ['admin', 'scout'];
  const GHOSTS = ['admin', 'ghost'];
  ...
  switch (req.url) {
    ...
    case (req.url.match(/artefacts/) || {}).input:
    case (req.url.match(/artesweeps/) || {}).input:
    case (req.url.match(/catapoints/) || {}).input:
    case (req.url.match(/treasuries/) || {}).input:
    ...
    case (req.url.match(/scouts/) || {}).input:
      if (
        !req.authorizedUser.roles.some(
          (r) => SCOUTS.includes(r) || MANAGERS.includes(r)
        )
      ) {
        res
          .status(HttpStatus.FORBIDDEN)
    ...
    case (req.url.match(/ghosts/) || {}).input:
      if (
        !req.authorizedUser.roles.some(
          (r) => GHOSTS.includes(r) || MANAGERS.includes(r)
        )
      ) {
        res
          .status(HttpStatus.FORBIDDEN)
