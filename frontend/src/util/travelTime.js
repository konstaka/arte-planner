...
export const getTravelTime = (target, attacker, map) => {
  ...
  let distance = Math.sqrt(
    Math.pow(
      Math.min(
        ...
        store.state.serverConfig.size * 2 +
          1 -
          Math.abs(attacker.xCoord - target.xCoord)
      ),
      2
  ...
  let squaresPerSecond =
    (attacker.unitSpeed * store.state.serverConfig.speed * attacker.arteSpeed) /
    60 /
    60;
...
export const getLandingTime = (target, attacker) => {
  ...
  if (target && attacker) {
    time.setSeconds(
      time.getSeconds() +
        (store.state.flexSeconds[target._id][attacker._id] || 0)
    );
  }
  return time;
};
...
export const getGhostSend = (target, attacker, ghost) => {
  ...
  if (getLandingTime(target, attacker).getTime() > ghostSend.getTime()) {
    ghostSend.setSeconds(ghostSend.getSeconds() + 1);
  }
  return ghostSend;
};
