<template>
  <div class="attacker_row">
    ...
    <div class="data_item without_hero">
      clears
      <DropDown
        v-model.number="mutableAttacker.clearWithoutHero"
        :options="arteSizes"
        :initial-value="attacker.clearWithoutHero"
      />
    </div>
    <div class="data_item with_hero">
      (
      <DropDown
        v-model.number="mutableAttacker.clearWithHero"
        :options="arteSizes"
        :initial-value="attacker.clearWithHero"
      />
      )
    </div>
...
export default {
  ...
  props: ['attacker', 'unitSpeeds', 'tsLevels', 'heroBoots', 'arteSizes'],
  data: () => ({
    mutableAttacker: {},
    loaded: false,
  }),
  ...
  methods: {
    async updateAttacker() {
      await api().put(`/artesweeps/${this.attacker._id}`, this.mutableAttacker);
      ...
      window.VoerroModal.show({
        ...
        buttons: [
          ...
          },
          {
            text: 'Delete',
            handler: async () => {
              await api().delete(`/artesweeps/${this.attacker._id}`);
              this.$store.dispatch('updateCycle');
            },
          },
        ],
