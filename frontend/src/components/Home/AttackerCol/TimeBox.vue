<template>
  <div v-if="!filtered || isVisible" class="timebox">
    <div class="times" :class="{ pressed: isSelected }" @click="toggleSelected">
      {{ sendingTime }} -> {{ landingTime }}
    </div>
    <div class="buttons">
      <div class="button" @click="changeFlex(-1)">-</div>
      <div class="button" @click="changeFlex(1)">+</div>
    </div>
  </div>
</template>
...
export default {
  name: 'TimeBox',
  props: ['target', 'attacker', 'filtered'],
  data: () => ({
    buttonMultiplier: 1,
    timeout: null,
  }),
  computed: {
    sendingTime() {
      ...
      return getSendingTime(this.target, this.attacker).toLocaleTimeString(
        'en-GB',
        options
      );
    },
    landingTime() {
      ...
      return getLandingTime(this.target, this.attacker).toLocaleTimeString(
        'en-GB',
        options
      );
    },
    ...
    isSelected() {
      return this.$store.state.selections.some((selection) => {
        return (
          selection.targetId === this.target._id &&
